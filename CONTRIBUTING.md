...
5. If you changed install, dependency, test, lint, or local setup behavior, update `.well-known/assistant-guide.txt`, the byte-identical root `assistant-guide.txt` copy, the Pages source copy at `docs/.well-known/assistant-guide.txt`, and both manifest copies in the same PR. Then run the full test suite so `tests/test_assistant_guide.py` verifies the GuideCheck byte profile, copy identity, and manifest hash.
